...
drop table rhdsrs.na_field_marketing.nafm_all;
drop table rhdsrs.na_field_marketing.nafm_all_flagged;
...
-- check for the old counts
  -- accounts
select count(*) from na_field_marketing.nafm_accounts_c;  -- 720,613,now 725k

--contacts
select count( distinct c_emailaddress) from na_field_marketing.nafm_contacts_c --2,024,391

-- NA Comm
select count( distinct c_emailaddress) from na_field_marketing.nafm_PS --2,024,391



-- NA PS
select count( distinct c_emailaddress) from na_field_marketing.nafm_PS --306,634, 307k


alter user nafmsio password 'S1Olocked'
...
create table na_field_marketing.nafm_all as (
select *
...
create table na_field_marketing.nafm_all_flagged as
(
select a.*,
...
from na_field_marketing.nafm_all a);
...
(
select *
from na_field_marketing.nafm_all_flagged
...
from na_field_marketing.nafm_all_flagged f
